 ...
 * 요청과 응답 객체 간 변환은 {@link MemberPresentationMapper}를 통해 처리합니다.
 *
...
public class MemberController {
    ...
    @PostMapping
    public ResponseEntity<MemberRegisterResponse> userSignUp(@Valid @RequestBody MemberRegisterRequest request) {
        MemberRegisterCommand command = toSignCommand(request);
        MemberResult result = memberRegisterService.userSignUp(command);
    ...
    public ResponseEntity<MemberResponse> getUserByEmail(@AuthenticationPrincipal LoginUser loginUser) {
        ...
        MemberResult result = memberQueryService.getUserByEmail(email);
