...
See 프로젝트는 소셜 블로깅 플랫폼으로, **도메인 주도 설계(DDD)**와 **헥사고날 아키텍처** 원칙을 적용하여 설계되었습니다. 비즈니스 복잡성을 도메인 모델로 명확하게 표현하고, 기술적 세부사항과 분리하여 유지보수성과 확장성을 확보했습니다.
...
stateDiagram-v2
    [*] --> DRAFT : create()
    [*] --> PUBLISHED : create(publishImmediately=true)
...
public void handlePostViewed(PostViewed event) {
    PostStats stats = postStatsRepository.findByPostId(event.postId())
        .orElseGet(() -> PostStats.create(event.postId()));
